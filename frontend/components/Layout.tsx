...
export default function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <nav className="bg-gradient-to-br from-indigo-600 to-violet-600 dark:from-indigo-800 dark:to-violet-800 text-white p-0 shadow-md sticky top-0 z-[100]">
        <div className="max-w-[1400px] mx-auto px-8 py-4 flex items-center justify-between">
          ...
          <div className={`flex gap-8 items-center max-md:absolute max-md:top-full max-md:left-0 max-md:right-0 max-md:bg-gradient-to-br max-md:from-indigo-600 max-md:to-violet-600 max-md:dark:from-indigo-800 max-md:dark:to-violet-800 max-md:flex-col max-md:p-4 max-md:gap-2 ${mobileMenuOpen ? 'max-md:flex' : 'max-md:hidden'}`}>
          ...
          <div className="flex gap-4 items-center max-md:gap-2">
            <Link href="/profile" className="w-10 h-10 rounded-full bg-white/20 dark:bg-white/10 flex items-center justify-center font-bold cursor-pointer transition-colors no-underline text-white hover:bg-white/30">
              {user?.email?.charAt(0).toUpperCase() || 'U'}
            </Link>
            <button onClick={handleLogout} className="bg-white/20 dark:bg-white/10 text-white border-0 px-6 py-2 rounded-lg cursor-pointer font-medium transition-colors hover:bg-white/30 max-md:px-4 max-md:text-sm">
              Logout
            </button>
          </div>
        </div>
      ...
      <main className="flex-1" style={{ backgroundColor: 'var(--bg-main)' }}>
        {children}
      </main>
    </div>
  )
